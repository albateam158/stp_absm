<@layout.extends name="layout/base.ftl">
                ...
                <div class="panel panel-transparent">
                    <div class="panel-header">
                        <div class="no-padding m-b-10">
                            <h3>프로젝트 업데이트</h3>
                            <p>WiCrowd 프로젝트 업데이트 상세 페이지 입니다.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover table-condensed table-detailed" id="commentTable">
                ...
                <#if update.comments?has_content>
                    <#list update.comments as comment>
                        ...
                        <#if !comment.deleteDate?exists && !comment.adminDeleteDate?exists>
                    <tr comment_id="${(comment.commentId) !}" replyString="${(comment.replyString?js_string?replace("\"", "\'")) !}">
                        ...
                        <td class="v-align-middle"><button class="btn btn-danger btn-xs m-t-10 comment_del_btn">Delete</button></td>
                    </tr>
                        </#if>
                    </#list>
                <#else>
    ...
    <script type="text/javascript">
        ...
        var projectId = '${(update.projectId) !}';
        ...
        $( document ).ready(function()
        {
            ...
            $('.go-list').click(function () {
                document.location.href = '/project/'+projectId+'/update';
            });
            ...
            $(document).on('click', '.comment_del_btn', function () {
                ...
                NoticeCommon.Dialog.confirm({
                    ...
                    ,ok: function(){
                        $.ajax({
                            url: "/project/"+projectId+"/update/"+updateId+"/comment/"+commentId,
                            type: "DELETE",
                            dataType: 'json',
                            data: '',
